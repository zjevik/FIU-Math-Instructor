...
function fillData() {
  ...
  $("#maindiv table tr").each(function(index) {
    ...
    if (!row.children[2].innerText.includes(" ")) {
      row.children[3].children[1].value = dic.hasOwnProperty(row.children[2].innerText.trim()) ? dic[row.children[2].innerText.trim()] : "DROP";
      delete dic[row.children[2].innerText.trim()];
    }
  })
